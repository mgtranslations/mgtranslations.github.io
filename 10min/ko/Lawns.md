#### 10분 대학™ · Clackamas County 마스터 가드너 협회와 OSU Extension 마스터 가드너™ 프로그램의 협력 및 지원
...
#### 오리건 주립대학교 확장 서비스(OSU Extension Service)는 모든 프로그램, 서비스, 활동 및 자료에서 인종, 피부색, 출신 국가, 종교, 성별, 성 정체성(성 표현 포함), 성적 지향, 장애, 나이, 결혼 여부, 가족/부모 상태, 공공 지원 프로그램에서 파생된 소득, 정치적 신념, 유전 정보, 군 복무 여부, 또는 이전 인권 활동에 대한 보복을 이유로 차별을 금지합니다. (모든 금지 사유가 모든 프로그램에 적용되는 것은 아닙니다.)
---